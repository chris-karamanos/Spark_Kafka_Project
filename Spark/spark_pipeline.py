...
#  Standardize station_id format
station_status_df = station_status_df.withColumn("station_id", trim(col("station_id")))
station_info_df = station_info_df.withColumn("station_id", trim(col("station_id")))

#  Drop duplicates (if any)
station_status_df = station_status_df.dropDuplicates(["timestamp", "station_id"])

#  Join station information with real-time station status using `station_id`
stations_joined_df = station_info_df.alias("info") \
    .join(station_status_df.alias("status"), col("info.station_id") == col("status.station_id"), "inner") \
    .select(
        col("status.timestamp"),
        col("info.station_id"),
        col("info.name"),
        col("info.capacity"),
        col("status.num_bikes_available"),
        col("status.num_docks_available")
    )

stations_joined_df = stations_joined_df.dropDuplicates(["timestamp", "station_id"])


#  Write the joined data to console (for debugging)
stations_joined_query = stations_joined_df.writeStream \
    .outputMode("append") \
    .format("console") \
    .option("checkpointLocation", "/tmp/stations_joined_checkpoint") \
    .start()

#  Keep the streaming process running
stations_joined_query.awaitTermination()
